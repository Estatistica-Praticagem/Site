...
$sql = "
  WITH ultima_execucao AS (
    SELECT
      MAX(CAST(timestamp_execucao AS TIMESTAMP)) AS max_execucao
    FROM
      `local-bliss-359814.wherehouse.dados_openweather_forecast`
    WHERE
  ...
  WHERE
    city_name = 'Rio Grande'
    AND CAST(timestamp_execucao AS TIMESTAMP) = (SELECT max_execucao FROM ultima_execucao)
  ORDER BY
    CAST(dt_txt AS TIMESTAMP)
